<section id="home" class="p-0">
    <div id="mainCarousel" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-inner">
            <div class="carousel-item active">
                <picture>
                    <source srcset="./assets/img/sliders/slider1-mobile.webp" media="(max-width: 767.98px)" alt="Soluciones en SAP Business One">
                    <img src="./assets/img/sliders/slider1.webp" class="d-block w-100" alt="Soluciones en SAP Business One">
                </picture>
                <div class="carousel-caption d-flex justify-content-center align-items-center h-25">
                    <div>
                        <h5 class="caption-title">Soluciones en SAP Business One</h5>
                        <p class="caption-subtitle">Implementación, soporte y desarrollo de AddOns.</p>
                    </div>
                </div>

            </div>
            <div class="carousel-item">
                <picture>
                    <source srcset="./assets/img/sliders/slider2-mobile.webp" media="(max-width: 767.98px)">
                    <img src="./assets/img/sliders/slider2.webp" class="d-block w-100" alt="Automatiza tus procesos">
                </picture>
                <div class="carousel-caption d-flex justify-content-center align-items-center h-25">
                    <div>
                        <h5 class="caption-title">Automatiza tus procesos</h5>
                        <p class="caption-subtitle">Integra tu ERP con eCommerce, CRM y más.</p>
                    </div>
                </div>
            </div>
            <div class="carousel-item">
                <picture>
                    <source srcset="./assets/img/sliders/slider3-mobile.webp" media="(max-width: 767.98px)">
                    <img src="./assets/img/sliders/slider3.webp" class="d-block w-100" alt="Tecnología Business Intelligence">
                </picture>
                <div class="carousel-caption d-flex justify-content-center align-items-center h-25">
                    <div>
                        <h5 class="caption-title">Tecnología Business Intelligence</h5>
                        <p class="caption-subtitle">Toma decisiones estratégicas con un eficiente análisis de datos.</p>
                    </div>
                </div>
            </div>
            <div class="carousel-item">
                <picture>
                    <source srcset="./assets/img/sliders/slider4-mobile.webp" media="(max-width: 767.98px)">
                    <img src="./assets/img/sliders/slider4.webp" class="d-block w-100" alt="Soporte Especializado">
                </picture>
                <div class="carousel-caption d-flex justify-content-center align-items-center h-25">
                    <div>
                        <h5 class="caption-title">Soporte Especializado</h5>
                        <p class="caption-subtitle">Atención inmediata y personalizada.</p>
                    </div>
                </div>
            </div>
        </div>
        <!-- Controles -->
        <button class="carousel-control-prev" type="button" data-bs-target="#mainCarousel" data-bs-slide="prev">
            <span class="carousel-control-prev-icon"></span>
        </button>
        <button class="carousel-control-next" type="button" data-bs-target="#mainCarousel" data-bs-slide="next">
            <span class="carousel-control-next-icon"></span>
        </button>
    </div>
</section>